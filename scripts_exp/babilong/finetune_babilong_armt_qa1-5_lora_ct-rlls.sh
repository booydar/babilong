# Remove LoRA on last segment, leave only on previous
#!/usr/bin/env bash
set -e
cd ../../
...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29005 run_finetuning_babilong_armt_hf_trainer_ct.py \
        --task_dataset $TASK_DATASET \
        --noise_dataset $NOISE_DATASET \
        --babi_path /home/jovyan/rmt/babilong/data/tasks_1-20_v1-2/en-10k \
        --output_dir /home/jovyan/rmt/runs/test/babilong_multitask/$MODEL_NAME/lr_${LR}_d${D_MEM}_${SCHEDULER}_adamw_wd1e-03_${MAX_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_bptt-${K2}_from_cpt_0-2_lora_ct-v3-rlls/run_$N \
