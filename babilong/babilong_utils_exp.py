    ...
    def get_sample_size(self):
        if isinstance(self.sample_size, list):
            if self.gen.random() > self.mixed_length_ratio:
                sample_size = self.gen.choice(self.sample_size)
            sample_size = max(self.sample_size)
        else:
            sample_size = self.sample_size
        
        if self.vary_length_pct is not None:
            if self.gen.random() > 0.9:
                cutoff_pct = self.vary_length_pct * np.random.random()
                sample_size = int(sample_size * (1 - cutoff_pct))
        return sample_size
