...
def main(
    ...
    memory_cell_cls, recurrent_wrapper_cls
) -> None:
    """
    ...
    if not api_url:
        if len(model_cpt) != 0:
            ...
            if d_mem is not None:
                ...
                # print('\nrecurrent_wrapper_cls: ', recurrent_wrapper_cls)
                ...
                memory_cell_cls = get_cls_by_name(args.memory_cell_cls)
                recurrent_wrapper_cls = get_cls_by_name(args.recurrent_wrapper_cls)
                cell = memory_cell_cls(model, num_mem_tokens=mem_size)
                model = recurrent_wrapper_cls(cell,
                                         segment_size=segment_size-2*mem_size,
                                         max_n_segments=max_n_segments,
                ).to(device)
...
if __name__ == '__main__':
    ...
    main(args.results_folder, args.model_name, args.tasks, args.lengths, args.dataset_name, args.use_chat_template,
         ...
         args.memory_cell_cls, args.recurrent_wrapper_cls)