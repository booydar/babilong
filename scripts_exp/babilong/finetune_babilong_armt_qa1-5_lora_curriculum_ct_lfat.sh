...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29006 run_finetuning_babilong_armt_hf_trainer_ct.py \
        ...
        --output_dir /home/jovyan/rmt/runs/test/babilong_multitask/$MODEL_NAME/lr_${LR}_d${D_MEM}_${SCHEDULER}_adamw_wd1e-03_${MAX_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_bptt-${K2}_from_cpt_${SRC_N_SEGMENTS}-${MAX_N_SEGMENTS}_lora_ct-v3-lfat/run_$N \
        --model_cpt /home/jovyan/rmt/runs/test/babilong_multitask/$MODEL_NAME/lr_3e-04_d${D_MEM}_${SCHEDULER}_adamw_wd1e-03_${SRC_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_bptt-${K2}_from_cpt_${SRC_SRC_N_SEGMENTS}-${SRC_N_SEGMENTS}_lora_ct-v3-lfat/run_$N \
