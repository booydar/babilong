...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29500 run_finetuning_pg19_rmt.py \
        --task_name $TASK_NAME \
        --model_path /home/jovyan/rmt/runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${BLOCK_SIZE}-${HISTORY_SIZE}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_${SRC_SRC_N_SEGMENTS}-${SRC_N_SEGMENTS}_from_dataset_${TASK_DATASET}/run_$N \
