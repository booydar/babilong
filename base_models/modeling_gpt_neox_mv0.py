...
class GPTNeoXLayer(nn.Module):
    ...
    def forward(
        ...
        has_memory = read_memory_state is not None and write_memory_state is not None
        if has_memory:
            hidden_states = torch.cat((read_memory_state, hidden_states, write_memory_state), dim=1)

            # if attention_mask is not None:
            #     # def pad_attention_mask(read_memory_state, hidden_states, write_memory_state):

            #     read_mem_attn_mask = torch.zeros(attention_mask.shape[0], 1, 1, read_memory_state.shape[1])
            #     write_mem_attn_mask = torch.zeros(attention_mask.shape[0], 1, 1, write_memory_state.shape[1])


            
            # # Create artificial position for all memory tokens
            # rm_pos_ids = torch.ones(read_memory_state.shape[1], dtype=torch.long, device=hidden_states.device) * 65536
            # wm_pos_ids = torch.ones(write_memory_state.shape[1], dtype=torch.long, device=hidden_states.device) * 65536
            # position_ids = torch.cat((rm_pos_ids, position_ids, wm_pos_ids))
        ...
        if has_memory:
            ...
            hidden_states = hidden_states[:, read_memory_state.shape[1] : -write_memory_state.shape[1]]
        ...
        if has_memory:
            ...
            # return outputs, read_memory_state, write_memory_state
        return outputs
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        memory_state: Optional[torch.FloatTensor] = None,
        # read_memory_state: Optional[torch.FloatTensor] = None,
        # write_memory_state: Optional[torch.FloatTensor] = None,
        ...
        if position_ids is None:
            ...
            if memory_state is not None:
                mem_pos_idx = 0
                memory_position_ids = torch.ones((1, memory_state.shape[1]), dtype=torch.long, device=device) * mem_pos_idx
        ...
        read_memory_state = write_memory_state = memory_state
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        result.memory_state = outputs.memory_state
        return result
