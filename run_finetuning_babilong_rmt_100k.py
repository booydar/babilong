...
if __name__ == '__main__':
    ...
    # task dataset 
    train_path = os.path.join(args.babi_path, f"{args.task_dataset}_train.npy")
    test_path = os.path.join(args.babi_path, f"{args.task_dataset}_test.npy")
    
    
    train_samples = np.load(train_path, allow_pickle=True)
    test_samples = np.load(test_path, allow_pickle=True)

    if args.max_n_facts is not None:
        train_samples = list(filter(lambda s: len(s['facts']) < args.max_n_facts, train_samples))
        test_samples = list(filter(lambda s: len(s['facts']) < args.max_n_facts, test_samples))

    task_dataset_train = datasets.Dataset.from_list(list(train_samples))
    task_dataset_test = datasets.Dataset.from_list(list(test_samples))
    ...
    def metrics_fn(data):
        ...
        if 'generation_outputs' in data:
            ...
            try:
                ...
                print("Got an error with data predictions:")
