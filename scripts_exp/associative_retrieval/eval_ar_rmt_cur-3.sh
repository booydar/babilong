...
NUMS_PAIRS=(1 2 3 5 10 20 40 50 100 200 300 400 500 700 1000)
...
python run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
        --model_path ../runs/test/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_k${KEY_SIZE}-v${VALUE_SIZE}-p${NUM_PAIRS}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_${NUM_LAYERS}l${NUM_LAYERS}hd${DIM}_eval_from_p${PREV_NUM_PAIRS}/run_$N \
        --model_cfg $MODEL_CFG \
        ...
        --batch_size $REAL_TBS --gradient_accumulation_steps 1 \
        --iters $ITERS \
